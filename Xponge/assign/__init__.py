...
connected to two other atoms, "N4" means a nitrogen atom connected to four other atoms.
    ...
    def add_bond(self, atom1, atom2, order=-1):
        ...
        if atom2 not in self.bonds[atom1]:
            self._bond_sequence.append((atom1, atom2))
...
0 ~ {self.atom_numbers}, but {atom} got")
    ...
    def delete_bond(self, atom1, atom2):
        ...
        self._bond_sequence = [
            pair for pair in self._bond_sequence if pair not in ((atom1, atom2), (atom2, atom1))
        ]
...
the marker "ar" will be added to the aromatic bond.
    ...
    def save_as_pdb(self, filename):
        ...
        :return: None
        """
        from ..build import _pdb_format_integer_field
        ...
        for i in range(self.atom_numbers):
            if self.names[i]:
                ...
                self.names[i] = atom_name
        for i, atom in enumerate(self.atoms):
            towrite += ("ATOM  {0:s} {1:4s} {2:3s} {3:1s}{4:s}    {5:8.3f}{6:8.3f}{7:8.3f}{8:22s}{9:2s}\n").format(
                _pdb_format_integer_field(5, i + 1, "atom serial"),
                self.names[i],
                self.name,
                " ",
                _pdb_format_integer_field(4, 1, "residue sequence number"),
                self.coordinate[i][0],
                self.coordinate[i][1],
        ...
        for i in range(self.atom_numbers):
            ...
            if bonded_atoms:
                for atoms in bonded_atoms:
                    towrite += "CONECT" + _pdb_format_integer_field(5, i + 1, "atom serial")
                    for atom in atoms:
                        towrite += _pdb_format_integer_field(5, atom + 1, "atom serial")
                    towrite += "\n"
...
If None is given, the total charge will not be checked
    ...
    from ..load import _pdb_parse_atom_serial
    with file as f:
        for line in f:
            if line.startswith("ATOM") or line.startswith("HETATM"):
                ...
                if only_residue:
                    if resname != only_residue:
                        continue
                assign.name = resname
                index = _pdb_parse_atom_serial(line[6:11])
                if index is None:
                    continue
            ...
            if line.startswith("CONECT"):
                has_conect = True
                atom = _pdb_parse_atom_serial(line[6:11])
                if atom is None:
                    continue
                ...
                for bonded_atom_i in range(11, 31, 5):
                    ...
                    bonded_atom = _pdb_parse_atom_serial(temp)
                    if bonded_atom is None:
                        break
                    if bonded_atom in index_atom_map.keys():
                        assign.Add_Bond(index_atom_map[atom], index_atom_map[bonded_atom])
