...
def _rule_sx(i, assign):
    ...
    :return:
    """
    tofind = assign.Atom_Judge(i, "S3")
    if tofind and "db" not in assign.atom_marker[i].keys():
        # Amber prefers sx for sulfoxide-like sulfur; bond-order inference may miss S=O.
        tofind = any(assign.Atom_Judge(bonded_atom, ["O1", "S1"]) for bonded_atom in assign.bonds[i].keys())
    if tofind:
        tofind = False
        for bonded_atom, bond_order in assign.bonds[i].items():
            if bond_order == 1 and (assign.Atom_Judge(bonded_atom, ["C3", "N2", "P2"]) or
