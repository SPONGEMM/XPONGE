...
def _is_ne_nf(i, assign):
    marker = assign.atom_marker[i]
    has_double_like_marker = "db" in marker or "DL" in marker or "DLB" in marker
    tofind = assign.Atom_Judge(i, "N2") and "sb" in marker and has_double_like_marker
    if not tofind:
        return False
    for bonded_atom, bond_order in assign.bonds[i].items():
        if bond_order == 1 and (assign.Atom_Judge(bonded_atom, ["C3", "C2", "N2", "P2"]) or
...
def _rule_c(i, assign):
    ...
    :return:
    """
    tofind = assign.Atom_Judge(i, "C3") and "AR1" not in assign.atom_marker[i]
    if tofind:
        tofind = False
        for bonded_atom in assign.bonds[i].keys():
