...
def test_bundle_wrapper_combines_protocol_and_authoritative_mapping(tmp_path):
    h5py = pytest.importorskip("h5py")
    molecule = _alanine_dipeptide()
    molecule.get_atoms()
    source_atom_ids = tuple(
        f"source:{index}" for index in range(len(molecule.atoms))
    )
    simulation_mapping = tuple(
        {
            "simulation_index": index,
            "external_id": source_atom_id,
            "canonical_atom_id": index + 1,
            "simulation_residue_index": 0,
            "simulation_residue_id": "residue:0",
        }
        for index, source_atom_id in enumerate(source_atom_ids)
    )
    ...
    returned, export_mapping = Xponge.save_sponge_input(
        molecule,
        "system",
        tmp_path,
        format="bundle",
        ...
        topology_hash="1" * 64,
        atom_order_hash="2" * 64,
        mapping_hash="3" * 64,
        protocol=protocol,
    )

    assert returned is molecule
    assert tuple(record["source_atom_id"] for record in export_mapping) == source_atom_ids
    with h5py.File(tmp_path / "system_topology.spgt.h5", "r") as handle:
        assert handle["/topology/mapping/external_id"].asstr()[:].tolist() == list(
            source_atom_ids
        )
        assert handle["/topology/mapping/mapping_hash"].asstr()[0] == "3" * 64
    with h5py.File(tmp_path / "system_protocol.spgp.h5", "r") as handle:
        assert handle["/cv/distance/type"].asstr()[()] == "distance"
...
def test_rmsd_reference_is_inline_in_protocol(tmp_path, use_atom_refs):
    ...
    Xponge.save_sponge_input_bundle(
        _alanine_dipeptide(), "rmsd", tmp_path, protocol=protocol
    )
...
def test_invalid_rmsd_reference_does_not_publish_bundle(tmp_path, cv_type, reference, error):
    ...
    with pytest.raises(BundleValidationError, match=error):
        Xponge.save_sponge_input_bundle(
            _alanine_dipeptide(), "invalid_rmsd", tmp_path, protocol=protocol
        )
